...
<p align="center">
  <img src="https://img.shields.io/badge/version-3.1.4-blue?style=flat-square" alt="Phiên bản">
...
- **[`Promise.any()`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise/any)** — Gửi yêu cầu đến cả hai điểm cuối API Bing đồng thời và dùng cái phản hồi trước, loại bỏ chờ đợi không cần thiết
- **[`AbortController`](https://developer.mozilla.org/docs/Web/API/AbortController)** — Giới hạn mỗi yêu cầu API Bing ở 8 giây, hủy kết nối thua một cách gọn gàng thay vì để nó treo đến hết thời gian chờ TCP ở cấp hệ điều hành
...
API Bing gửi yêu cầu đến cả hai điểm cuối đồng thời qua `Promise.any` với thời gian chờ 8 giây qua `AbortController` — phản hồi nhanh nhất thắng. Tải trọng JSON rất nhỏ, nên yêu cầu thêm hầu như không tốn chi phí, nhưng cuộc đua đảm bảo độ trễ tối ưu bất kể bạn ở đâu. Mã ngôn ngữ (ví dụ `zh-CN`) được ánh xạ sang mã thị trường Bing, một số ngôn ngữ rơi về `en-US`.
