...
Bing API には二つのエンドポイントが用意されており、プライマリ・フォールバックの冗長構成をとる。言語コード（例：`zh-CN`）は Bing のマーケットコードにマッピングされ、一部の言語は `en-US` にフォールバックする。
